	...
	for key, row in pairs(destinations) do
		...
		--process according to user_exists, sip_uri, external number
			...
			elseif (tonumber(destination_number) == nil) then
				...
				--for inbound calls forwarded to external, use B party DID as CID
				--so C party sees B's number, not A's number
				--ensure NO A party data leaks to gateway
					if (call_direction == "inbound" and caller_destination ~= nil and caller_destination ~= "") then
						caller_id_name = caller_destination;
				...
				--for forwarded inbound calls, also set outbound/effective CID and SIP headers
				--so the outbound route and gateway both use B party DID
					if (call_direction == "inbound" and caller_destination ~= nil and caller_destination ~= "") then
						caller_id = caller_id .. ",outbound_caller_id_name='"..caller_destination.."'";
						caller_id = caller_id .. ",outbound_caller_id_number="..caller_destination;
						caller_id = caller_id .. ",effective_caller_id_name='"..caller_destination.."'";
						caller_id = caller_id .. ",effective_caller_id_number="..caller_destination;
						caller_id = caller_id .. ",sip_cid_type=none";
					end
